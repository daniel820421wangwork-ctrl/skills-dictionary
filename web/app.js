...
(function () {
  "use strict";
  var SKILLS = [];   // 由 IndexedDB 載入（種子來自 data/skills.js）
  var TAGS = {};
  ...
  function render() {
    ...
    resultCount.textContent = "顯示 " + list.length + " / " + SKILLS.length + " 個 skill";
  ...
  function openModal(id) {
    ...
    if (bl) {
      ...
      var headTxt = act === "block"
        ? "⚠️ 此 skill 已被封鎖・" + sev.label + "（不會被推薦）"
  ...
  function buildSuggestChips() {
    var ex = ["使用者登入驗證", "串接 Stripe 金流", "部署到 Cloudflare", "爬網頁資料",
  ...
  function renderBlModal() {
    ...
    $("bl-body").innerHTML =
      '<div class="m-head"><h2 id="bl-title">⚠️ 黑名單管理</h2></div>' +
      '<p class="bl-note">命中黑名單的 skill 會在卡片/詳情標警告，且<b>不會被對話式搜尋推薦</b>。' +
        '清單存在你瀏覽器的 IndexedDB，<b>可自行增刪改</b>，不會上傳。預設項目取自公開資安通報，' +
        '請依實際情況維護；新增時務必附上可查證的來源。</p>' +
      '<div class="bl-list">' + rows + '</div>' +
      '<h3 class="bl-add-title">新增項目</h3>' +
      '<div class="bl-form">' +
        '<input id="bl-in-pat" class="bl-in" type="text" placeholder="比對規則：vendor/skill（單一）或 vendor（整個發布者）">' +
    ...
    Array.prototype.forEach.call($("bl-body").querySelectorAll(".bl-del"), function (b) {
      b.addEventListener("click", function () {
        SkillDB.removeBlacklist(b.getAttribute("data-pat")).then(refreshBlacklist).then(renderBlModal);
      });
    });
    ...
    $("bl-reset").addEventListener("click", function () {
      ...
      SkillDB.resetBlacklist().then(refreshBlacklist).then(renderBlModal);
    });
  }
  ...
  function onBlAdd() {
    ...
    SkillDB.putBlacklist(entry).then(refreshBlacklist).then(renderBlModal);
  }

  // 重新讀取黑名單並更新整個畫面
  function refreshBlacklist() {
    return SkillDB.getBlacklist().then(function (list) {
      BLACKLIST = list || [];
      buildBlControls();
      render();
    });
  ...
  function buildStats() {
    ...
    $("stats").innerHTML =
      '<div class="stat"><span class="num">' + SKILLS.length + '</span><span class="lbl">公開技能</span></div>' +
    ...
    $("foot-count").textContent = SKILLS.length;
  }
  ...
  function init() {
    ...
    var built = false;
    function buildOnce(skills, tags) {
      if (built) return;
      built = true;
      SKILLS = sortSkills((skills && skills.length ? skills : (window.SKILLS || [])).slice());
      TAGS = tags && Object.keys(tags).length ? tags : (window.TAGS || {});
    ...
    var timer = setTimeout(function () {
      ...
      buildOnce(window.SKILLS, window.TAGS);
    }, 1500);

    SkillDB.init()
      .then(function () { return Promise.all([SkillDB.getAllSkills(), SkillDB.getTags(), SkillDB.getBlacklist()]); })
      .then(function (res) {
        clearTimeout(timer);
        BLACKLIST = res[2] || [];
        buildOnce(res[0], res[1]);
